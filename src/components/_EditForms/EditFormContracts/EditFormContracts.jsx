...
import { StyledButton, StyledInputBox, StyledInput, ErrorMessage, StyledFormWrapper, StyledForm } from "@GlobalComponents";
import { Context } from "@root";
import { setBaner } from "@setBaner";

function EditFormContracts() {
  const context = useContext(Context);
  const { credentialsContracts, setCredentialsContracts, setEditBaner } = context;
  ...
  return (
    <Formik
      ...
      onSubmit={(values) => {
        ...
        const toUptade = Object.keys(credentialsContracts)[indexOfEditedData];
        update(ref(db, `files/contracts/${toUptade}`), values);
...
export default EditFormContracts;
